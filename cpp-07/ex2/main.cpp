...
int main(int argc, char** argv)
{
    ...
    std::map<std::string, std::variant<std::string, int>> attack_res = warrior.attack("Enemy");
    std::cout << "Attack result: ";
    print_dict(attack_res);

    std::map<std::string, std::variant<std::string, int>> defend_res = warrior.defend(5);
    std::cout << "Defense result: ";
    print_dict(defend_res);

    std::cout << "Magic phase:" << std::endl;
    std::map<std::string, std::variant<std::string, int>> spell_res = warrior.cast_spell("Fireball", {"Enemy1", "Enemy2"});
    std::cout << "Spell cast: ";
    print_dict(spell_res);

    std::map<std::string, std::variant<std::string, int>> channel_res = warrior.channel_mana(3);
