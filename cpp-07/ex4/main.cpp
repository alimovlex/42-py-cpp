...
int main() {
    ...
    auto dragon = std::make_shared<TournamentCard>("Fire Dragon", 5, "Legendary", 7, 5, "dragon_001");
    auto wizard = std::make_shared<TournamentCard>("Ice Wizard", 4, "Rare", 3, 4, "wizard_001");
    ...
    auto dragon_stats = dragon->get_tournament_stats();
    ...
    std::cout << "- Interfaces: [Card, Combatable, Rankable]" << std::endl;
    auto wizard_stats = wizard->get_tournament_stats();
    ...
    auto match_result = platform.create_match("dragon_001", "wizard_001");
    std::cout << "Match result: ";
    print_dict(match_result);

    std::cout << "Tournament Leaderboard:" << std::endl;
    auto leaderboard = platform.get_leaderboard();
    int rank = 1;
    ...
    auto report = platform.generate_tournament_report();
    print_dict(report);
