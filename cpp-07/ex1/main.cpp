...
int main(int argc, char** argv)
{
    ...
    std::map<std::string, std::variant<std::string, int>> stats = deck.get_deck_stats();
    std::cout << "Deck stats: ";
    print_dict(stats);
    ...
    for (int i = 0; i < 3; ++i)
    {
        std::unique_ptr<Card> card = deck.draw_card();
        if (card)
        {
            ...
            std::map<std::string, std::variant<std::string, int>> result = card->play(game_state);
